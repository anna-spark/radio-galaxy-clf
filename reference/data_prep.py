...
def load_galaxy_data(augment=False, augment_interval=60):
    """
    Loads galaxy data from FITS files, prepares labels, and optionally performs augmentation.
    
    Saves:
        - data/galaxy_X.npy
        - data/galaxy_y.npy
        - data/galaxy_y_aux.npy
    """
    rootdir = '../../FITS/'
    subdirs = ['BENT', 'COMP', 'FRI', 'FRII']
    label_dict = {'BENT': [1, 0, 0, 0],
                  'COMP': [0, 1, 0, 0],
                  'FRI':  [0, 0, 1, 0],
                  'FRII': [0, 0, 0, 1]}
    ...
    for sub in subdirs:
        ...
        if len(files) == 0:
            warnings.warn(f'No files found for class {sub}', RuntimeWarning)
            continue
        
        for file in files:
            # Skip known corrupt files
            if 'BENT_WAT_106.fits' in file:
                continue

            try:
                img = fits.open(file)[0].data.astype(np.float32)
            except Exception as e:
                warnings.warn(f'Failed to load {file}: {e}', RuntimeWarning)
                continue

            # Crop to central 150x150 region
            focus = img[75:225, 75:225].copy()
            
            # Ensure proper float range
            focus = np.nan_to_num(focus, nan=0.0, posinf=0.0, neginf=0.0)
            
            # Extract auxiliary features if process_input exists
            try:
                fr_ratio, cores, core_frac, pb = process_input(focus, -1)
                bent = 1 if pb else 0
                feats = [bent, fr_ratio, cores, core_frac]
            except NameError:
                feats = [0,0,0,0]  # fallback if process_input not defined
            ...
            if augment:
                augmented_imgs = augment_galaxy_data(focus, r=augment_interval)
                for aug_img in augmented_imgs[1:]:  # skip 0° since already added
                    X.append(aug_img)
                    y.append(label_dict[sub])
                    y_aux.append(feats)

    # Convert to arrays
    X = np.array(X, dtype=np.float32)[..., np.newaxis]  # add channel dim for CNN
    y = np.array(y, dtype=np.uint8)
    y_aux = np.array(y_aux, dtype=np.float32)
    ...
    print(f"✅ Loaded {len(X)} samples across {len(subdirs)} classes.")
...
def process_input(img, i):
    ...
    # If thresholding failed and returned empty, skip
    if np.sum(threshed) == 0:
        print(f"Warning: No pixels detected in thresholded image {i}")
        return 0, 0, 0, False

    cores, inter_dist, core_pixels, total_pixels = detect_cores(img, threshed)

    # Avoid division by zero
    core_frac = core_pixels / total_pixels if total_pixels > 0 else 0
    ...
    # If rotated is empty (no galaxy pixels), return zeros
    if rotated.size == 0:
        print(f"Warning: No galaxy pixels after rotation for image {i}")
        return 0, cores, core_frac, False
    ...
    if v_size == 0:
        print(f"Warning: Zero vertical size detected for image {i}")
        return cores, 0, core_frac, False
...
def calc_core_dist(coords):
    '''
    This function calculates the distance between the cores.
    ...
    '''
    if len(coords) == 0:
        # No cores detected, return 0
        print("Warning: No cores detected in this image.")
        return 0.0
